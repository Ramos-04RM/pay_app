...
from apps.services.forms import PayForm
...
from apps.cabinets.models import Cabinet, CabinetTag
from apps.tags.models import Tag
from pay_app.security import decrypt_value
from apps.common.constants import (
    PAY_MODE_ALL, PAY_MODE_UPCOMING, PAY_MODE_OVERDUE, PAY_MODES,
    PAY_SORT_MAP, PAY_SORT_DEFAULT, STAT_PERIOD_MONTH, STAT_PERIOD_YEAR,
    STAT_PERIOD_LAST_12, STAT_PERIOD_CUSTOM, STAT_PERIOD_CHOICES,
)
from apps.common.dates import add_one_month, _start_of_month, _end_of_month
from apps.core.context import get_common_context
...
def cabinet_edit(request, id):
    ...
    return render(request, 'cabinets/index.html', content)
...
def statistics_page(request):
    ...
    content.update(get_common_context())
    return render(request, 'statistics/index.html', content)
...
def tags_page(request):
    ...
    content.update(get_common_context())
    content.update(get_default_pay_context(request))
    return render(request, 'tags/list.html', content)
...
def tag_create(request):
    ...
    content.update(get_common_context())
    content.update(get_default_pay_context(request))
    return render(request, 'tags/form.html', content)
...
def tag_delete(request, id):
    ...
    content.update(get_common_context())
    content.update(get_default_pay_context(request))
    return render(request, 'tags/confirm_delete.html', content)
